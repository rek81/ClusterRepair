...
LocalPoint PixelCPEClusterRepair::localPosition(DetParam const& theDetParam, ClusterParam& theClusterParamBase) const {
  ...
  if (mcol > 3 && mrow <= 4 && int(ttopo_.layer(id)) == 1 && id.subdetId() == PixelSubdetector::PixelBarrel){
    std::cout << "starting to split cluster here" << std::endl;
  }
  for (int i = 0; i != theClusterParam.theCluster->size(); ++i) {
    auto pix = theClusterParam.theCluster->pixel(i);
    ...
    if ((irow < mrow) & (icol < mcol))
      ...
      //      std::cout << "cluster matrix in pix.adc statement is " <<  clustMatrix[irow][icol] << std::endl;
	...
	std::cout << "clustMatrix for mcol/2+1 is " << clustMatrix[irow][icol] << std::endl;
      ...
      //      std::cout << "clustersize is " << theClusterParam.theCluster->size() << std::endl;
                  std::cout << "irow is " << irow << std::endl;
                  std::cout << "icol is " << icol << std::endl;
                  std::cout << "mrow is " << mrow << std::endl;
                  std::cout << "mcol is " << mcol << std::endl;
      //      std::cout << "ydouble is  " << ydouble[icol] << std::endl;
      //      std::cout << "xdouble is  " << xdouble[icol] << std::endl;
      
    }
    else{ continue;
      //      std::cout << "clustMatrix after cont is " << clustMatrix[irow][icol] << std::endl;
    }
  }
  if (mcol > 3 && mrow <= 4 && int(ttopo_.layer(id)) == 1 && id.subdetId() == PixelSubdetector::PixelBarrel){
