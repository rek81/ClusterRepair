...

{
  ...
  if (id > 0) {  //if id == 0 bypass interpolation (used in calibration)
    if (!templ.interpolate(id, cotalpha, cotbeta, locBz, locBx)) {
      ...
      std::cout << "error 20 returned" << std::endl;
      return 20;
    }
  }
  ...
  if (deadpix) {
	...
	std::cout << "error 11 returned" << std::endl;
        return 11;
      }
	...
	std::cout << "error 12 returned" << std::endl;
        return 12;
      }
  ...
  if ((lypix - fypix + 1) != nypix || nypix == 0) {
    LOGDEBUG("SiPixelTemplateReco") << "y-length of pixel cluster doesn't agree with number of pixels above threshold"
                                    << ENDL;
    std::cout << "y-length of pixel cluster doesn't agree with number of pixels above threshold: length " << lypix - fypix + 1 << ", and pixel length is "<<nypix <<  std::endl;
    if (theVerboseLevel > 2) {
    ...
    //    std::cout << "error 1 returned" << std::endl;

    return 1;
  }
  else { 
    std::cout << "no error 1 returned" << std::endl;
    std::cout << "nypix is " << nypix << std::endl;
    std::cout << "nxpix is " << nxpix << std::endl;
    
  }


  //  else {
  //      std::cout << "Cluster is of fine length: length is " << lypix - fypix + 1 << ", and pixel length is " << nypix <<  std::endl;
  //    }
  // If cluster is longer than max template size, technique fails

  if (nypix > TYSIZE) {
    std::cout << "y-length of pixel cluster is larger than maximum template size " << nypix << std::endl;
    ...
    std::cout << "error 6 returned" << std::endl;
    return 6;
  }
  ...
  if (fytmp <= 1) {
    std::cout << "error 8 returned" << std::endl;
    return 8;
  }
  if (lytmp >= BYM2) {
    std::cout << "error 8 returned" << std::endl;
  ...
  if ((lxpix - fxpix + 1) != nxpix) {
    ...
    std::cout << "error 2 returned" << std::endl;
    return 2;
  }
  ...
  if (nxpix > TXSIZE) {
    ...
    std::cout << "error 7 returned" << std::endl;
    return 7;
  }
  ...
  if (fxtmp <= 1) {
    std::cout << "error 9 returned" << std::endl;
    return 9;
  }
  if (lxtmp >= BXM2) {
    std::cout << "error 9 returned" << std::endl;
  ...
  //  std::cout << "error 0 returned" << std::endl;

  return 0;
  //  if (not 20 || 11 || 12 || 1 || 2 || 6 || 7 || 8 || 9){
  //    }
}  // PixelTempReco1D
